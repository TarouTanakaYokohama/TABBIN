...
function createChromeMock(initialStore: LocalStore = {}) {
  ...
  const get = vi.fn(async (key?: string) => {
    ...
    return { [key]: structuredClone(store[key as keyof LocalStore]) }
  })
  ...
  return { store, get, set, sendMessage }
}
...
describe('tab-operations ユーティリティ', () => {
  ...
  describe('safelyUpdateGroupUrls関数', () => {
      ...
      const { store, set, sendMessage } = createChromeMock({
        savedTabs: [
          ...
          },
          {
            id: 'group-2',
            domain: 'other.com',
            urls: [{ url: 'https://other.com', title: 'B' }],
          },
        ],
      })
    ...
    it('URLs が残る場合はメッセージ送信せずにグループの URLs を更新する', async () => {
      ...
      const nextUrls = [{ url: 'https://example.com/next', title: 'Next' }]
      ...
      expect(set).toHaveBeenCalledWith({
        savedTabs: [{ id: 'group-1', domain: 'example.com', urls: nextUrls }],
      })
      expect(sendMessage).not.toHaveBeenCalled()
    })
