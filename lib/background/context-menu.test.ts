...
function createChromeHarness(
  ...
  return { create, removeAll, addListener, listeners }
}
...
describe('createContextMenus関数', () => {
  ...
  it('contextMenus API が利用できない場合にエラーをログ出力する', () => {
    createChromeHarness({ withContextMenus: false })
  ...
  it('runtime.lastError の削除エラーをログ出力して継続する', () => {
    const harness = createChromeHarness({
      runtimeLastError: { message: 'remove failed' },
    })
  ...
  it('メニュー削除セットアップ中の例外を捕捉する', () => {
    ...
    createChromeHarness({ removeAllThrows: error })
  ...
  it('メニュー項目作成中の例外を捕捉する', () => {
    ...
    createChromeHarness({ createThrows: error })
